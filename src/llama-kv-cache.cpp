...
template<typename T, bool causal, bool swa, bool is_2d, bool alibi, bool tree_attn>
...
static void set_input_kq_mask_impl(const args_set_input_kq_mask & args, T * data) {
    ...
    if (alibi) {
        if (tree_attn) {
            ...
            set_input_kq_mask_impl<T, causal, swa, is_2d, true, false> (args, data);
        }
    } else {
        if (tree_attn) {
            set_input_kq_mask_impl<T, causal, swa, is_2d, false, true> (args, data);
        } else {
            set_input_kq_mask_impl<T, causal, swa, is_2d, false, false> (args, data);
        }
    }
}
